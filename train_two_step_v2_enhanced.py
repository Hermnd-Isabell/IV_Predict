...
def fit_mw_surface(df_day: pd.DataFrame, call_put: str,
    ...
    按last_edate分组，每组沿M方向拟合LSQUnivariateSpline(M, W)。
    通过减少内部节点数强制平滑，避免精确插值导致residual恒为0。
    ...
    for mat, df_mat in df_sub.groupby('last_edate'):
        ...
        n = len(Mu)
        if n < 4:
            # 合约数不足，跳过该到期月（不拟合，baseline设为NaN后续填充）
            continue
        ...
        try:
            ...
            print(f"[WARN] LSQUnivariateSpline failed for mat={mat}, n={n}, fallback to UnivariateSpline: {e}")
        ...
        # 平坦外推：M在范围外时用边界值
        W_interp = np.where(M_query < Mu[0], Wu[0], W_interp)
        W_interp = np.where(M_query > Mu[-1], Wu[-1], W_interp)
...
def build_target(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 方案A: 预测残差绝对值 residual_{t+1}（prompt推荐优先尝试）
...
# Step 5: 模型训练
# =============================================================================
def train_delta_residual_model(X_train, y_train, X_val, y_val, sample_weight=None, param_grid=None):
...
def main():
    ...
    # ------------------------------------------------------------------
    mw_cache_path = os.path.join(OUTPUT_DIR, 'mw_checkpoint_v2.pkl')
    ...
    # ------------------------------------------------------------------
    # [Checkpoint 3] 增强数据集加载
    # ------------------------------------------------------------------
    print("\n[Checkpoint 3] 加载增强数据集")
    X, y, w, df = get_training_data_for_xgboost(r'E:\Codes\IV_Predict\data\enhanced_dataset_v2.pkl')

    print(f"  - 增强样本数: {len(df)}")
    print(f"  - 特征维度: {len(FEATURE_COLS)}")
    print(f"  - 目标变量统计: mean={y.mean():.6f}, std={y.std():.6f}")
    print(f"  - 真实样本数: {(w==1.0).sum()}, 合成样本数: {(w<1.0).sum()}")
    print(f"  - 前3行样本:")
    ...
    # ------------------------------------------------------------------
    print("\n[Checkpoint 5] 模型训练完成")
    model, best_params = train_delta_residual_model(X_train, y_train, X_val, y_val, sample_weight=w_train)
    ...
    try:
        ...
        dates, regime_ids = dates[:n], regime_ids[:n]
        regime_map = {int(d): int(r) for d, r in zip(dates, regime_ids)}
        test_df_pred['regime_id'] = test_df_pred['trade_date'].map(regime_map)
        for regime in [0, 1, 2]:
            mask = test_df_pred['regime_id'] == regime
            if mask.sum() > 0:
                rmse = np.sqrt(mean_squared_error(
                    test_df_pred.loc[mask, 'true_iv'],
                    test_df_pred.loc[mask, 'pred_iv']
                ))
                n_regime = mask.sum()
                print(f"    Regime {regime}: RMSE={rmse:.5f}, n={n_regime}")
            else:
                print(f"    Regime {regime}: n=0 (no test samples)")
    except Exception as e:
        print(f"    [Warning] Regime evaluation failed: {e}")
